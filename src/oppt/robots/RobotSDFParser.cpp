...
namespace oppt
{
RobotSDFParser::RobotSDFParser()
{

}

VectorGeometryUniquePtr RobotSDFParser::parseCollisionGeometries(const std::string& robotName, const std::string &environmentFile) const
{
...
VectorGeometryUniquePtr RobotSDFParser::parseVisualGeometries(const std::string& robotName, const std::string &environmentFile) const
{
    VectorGeometryUniquePtr robotGeometries;
    if (environmentFile.empty())
        return std::move(robotGeometries);
...
void RobotSDFParser::processMaterialElement(const sdf::ElementPtr& visualElement, geometric::Geometry *geometryElem) const
{
    if (visualElement) {
...
geometric::Pose RobotSDFParser::processPose(const sdf::ElementPtr &visualElement) const {
    geometric::Pose pose;
    if (visualElement) {
        sdf::ElementPtr visualPoseElem = visualElement->GetElement("pose");
        if (visualPoseElem) {
            std::string poseStr = visualPoseElem->GetValue()->GetAsString();
            VectorString poseElems;
            split(poseStr, ' ', poseElems);
            VectorFloat poseVec(6, 0.0);
            for (unsigned int i = 0; i != 6; ++i) {
                poseVec[i] = atof(poseElems[i].c_str());
            }
...
GeometryUniquePtr RobotSDFParser::processBoxGeometry(sdf::ElementPtr boxGeometryElem,
...
GeometryUniquePtr RobotSDFParser::processSphereGeometry(sdf::ElementPtr sphereGeometryElem,
        const std::string& visualName,
        const sdf::ElementPtr& visualElement) const
{
    ...
    GeometryUniquePtr sphereGeometry(new geometric::Sphere(visualName, radius, processPose(visualElement)));
    processMaterialElement(visualElement, sphereGeometry.get());
    return std::move(sphereGeometry);
}

GeometryUniquePtr RobotSDFParser::processCylinderGeometry(sdf::ElementPtr cylinderGeometryElem,
        const std::string& visualName,
        const sdf::ElementPtr& visualElement) const
{
    ...
    GeometryUniquePtr cylinderGeometry(new geometric::Cylinder(visualName, radius, length, processPose(visualElement)));
    processMaterialElement(visualElement, cylinderGeometry.get());
    return std::move(cylinderGeometry);
}

GeometryUniquePtr RobotSDFParser::processMeshGeometry(sdf::ElementPtr meshGeometryElem,
        const std::string& visualName,
        const sdf::ElementPtr& visualElement) const
{
    ...
    GeometryUniquePtr meshGeometry(new geometric::Mesh(visualName, meshUri, processPose(visualElement), scale));
