...
void SceneImpl::addBody(BodyUniquePtr body, bool executeCallback)
{
    ...
    Body* obst = getBody(body->getScopedName());
    if (obst) {
        removeBody(obst->getScopedName());
    }
    ...
    if (bodyPtr->isEnabled()) {
        ...
        for (auto &opptCollisionObject : opptCollisionObjects) {
            ...
            collisionObjectMap_[opptCollisionObject] = bodyPtr->getScopedName();
        }
...
void SceneImpl::removeModel(const std::string &modelName) {
    for (size_t i = bodies_.size(); i--;) {
        ...
        split(bodies_[i]->getScopedName(), "::", nameElems);
        if (nameElems[0] == modelName) {
            removeBody(bodies_[i]->getScopedName(), false);
        }
    }
}
...
bool SceneImpl::removeBody(std::string bodyName, bool executeCallback)
{
    ...
    for (size_t i = 0; i < bodies_.size(); i++) {
        bool found = false;
        std::string obstName = bodies_[i]->getScopedName();
        if (bodies_[i]->getScopedName() == bodyName) {
            found = true;
        ...
        if (found) {
            ...
            if (bodyManaged(body->getScopedName())) {
                ...
                sceneCollisionManager_->update();
                bodiesInManager_.erase(body->getScopedName());
            }
...
CollisionReportSharedPtr SceneImpl::makeDiscreteCollisionReport(const CollisionRequest *collisionRequest)
{
    ...
    for (size_t i = 0; i != collisionData.o1.size(); ++i) {
        ...
        collisionReport->collisionPairs.push_back(
            std::pair<std::string, std::string>(((OpptCollisionObjectData *)(collisionData.o2[i]->getUserData()))->opptCollisionObject->getCollisionGeometry()->getScopedName(),
                                                ((OpptCollisionObjectData *)(collisionData.o1[i]->getUserData()))->opptCollisionObject->getCollisionGeometry()->getScopedName()));
        ...
        for (size_t j = 0; j != collisionRequest->tfGoal.size(); j++) {
            for (auto &opptCollisionObject : opptCollisionObjects) {
                ...
                if (result.is_collide) {
                    ...
                    collisionReport->collisionPairs.push_back(
                        std::pair<std::string, std::string>({robotCollisionObjects_[j]->getCollisionGeometry()->getScopedName(),
                                                            opptCollisionObject->getCollisionGeometry()->getScopedName()
                                                            }));
                }
            }
        }
...
void SceneImpl::setRobotCollisionObjects(VectorCollisionObjectPtr& robotCollisionObjects)
{
    ...
    for (auto & robotCollisionObject : robotCollisionObjects_) {
        ...
        collisionObjectMap_[robotCollisionObject] = robotCollisionObject->getCollisionGeometry()->getScopedName();
    }
}
