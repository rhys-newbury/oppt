 ...
 */
class BodyAddedChangeSDF: public EnvironmentChange
{
public:
    using EnvironmentChange::EnvironmentChange;
    _NO_COPY_BUT_MOVE(BodyAddedChangeSDF)
    BodyAddedChangeSDF(const std::string& sdfString, const bool &requiresCallback = false):
        EnvironmentChange(requiresCallback),
        sdfString_(sdfString) {

    }

    virtual ~BodyAddedChangeSDF() = default;

    virtual EnvironmentChangeType getType() const override {
        return EnvironmentChangeType::BODY_ADDED_SDF;
    }
...
public:
    ...
    BodyAddedChange(BodyUniquePtr body, const bool &requiresCallback = false):
        EnvironmentChange(requiresCallback),
        body_(std::move(body)) {
