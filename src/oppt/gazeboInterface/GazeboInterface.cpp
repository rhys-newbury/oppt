            ...
            if (bodyEvent.first == "add") {
                ...
                for (auto & model : models) {
                    ...
                    if (model->GetName() == bodyName) {
                        ...
                        suppressOnAddBody_ = true;
                        addBodyFromSDFString(sdfString);
                        suppressOnAddBody_ = false;
...
void GazeboInterface::getObservationLimits(VectorFloat& lowerObservationLimits,
        VectorFloat& upperObservationLimits,
        const bool &ignoreObservationLimits) {
...
GazeboPropagationResultUniquePtr GazeboInterface::doPropagation(const GazeboPropagationRequest * const propagationRequest) {
    ...
    auto jointEntries = worldJointMap_.at(worldName);
    auto linkEntries = worldRobotLinkMap_.at(worldName);
    bool breaking = false;
