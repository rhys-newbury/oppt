      ...
      if (!bb->island_tag) {
        if (!(bb->flags & dxBodyDisabled)) {
          ...
          *bodycurr++ = bb;
          ...
          int bcount = bodycurr - bodiestart;
          ...
          *islandreqscurr = stepperestimate(bodiestart, bcount, jointstart, jcount);
                        ...
                        int jcount)
{
    ...
    BEGIN_STATE_SAVE(island_context, island_stepperstate) {
      stepper (island_context,world,bodiestart,bcount,jointstart,jcount,stepsize);
...
#define USE_TPISLAND
    ...
    if (world->threadpool && world->threadpool->size() > 0)
      world->threadpool->schedule(boost::bind(dxProcessOneIsland,island_context, world, stepsize, stepper,bodiestart, bcount, jointstart, jcount));
    else //automatically skip threadpool if only 1 thread allocated
      dxProcessOneIsland(island_context, world, stepsize, stepper,bodiestart, bcount, jointstart, jcount);
#else
    dxProcessOneIsland(island_context, world, stepsize, stepper,bodiestart, bcount, jointstart, jcount);
#endif
