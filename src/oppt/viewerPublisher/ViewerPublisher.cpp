    ...
    if (tmp) {
        ...
        if (ros::master::check()) {
            ...
            updateMarkersPublisher_ =
                node.advertise<visualization_msgs::MarkerArray>("viewerTopic", 100, false);
    ...
    if (foundROSEnvironment_) {
        ...
        for (size_t i = 0; i != poses.size(); ++i) {
            for (size_t j = 0; j != poses[i].size(); ++j) {
                if (geometries[j]->getWorldPose() != poses[i][j]) {
        ...
        if (geometryMarkers->markers.size() != robotMarkers_->markers.size())
            removeMarkersWithName(robotMarkers_->markerNames, robotMarkers_, true);
        boost::recursive_mutex::scoped_lock scoped_lock(environmentMtx_);
