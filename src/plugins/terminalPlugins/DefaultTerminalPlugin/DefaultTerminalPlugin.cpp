...
public :
    ...
    virtual ValidityReportSharedPtr isValid(const PropagationResultSharedPtr& propagationResult) override {
        ...
        validityReport->satisfiesConstraints =
            robotEnvironment_->getRobot()->getStateSpace()->insideStateLimits(*(propagationResult->nextState.get()));
        if (!validityReport->satisfiesConstraints) {
            if (robotEnvironment_->isExecutionEnvironment())
                WARNING("State doesn't satisfy constraints");
